...
class MoshiOnlyEngineWithHidden(MoshiOnlyEngine):
    ...
    def reset_session(self) -> None:
        ...
        # Tracks which turn_id has already had its AUDIO_FIRST_CHUNK_READY /
        # AUDIO_QUEUE_ENTER marks fired. Compared to _latency_turn_id each time
        # audio is enqueued; when they differ a new turn has started and the
        # first-audio mark fires once and updates this field. Reset to None
        # here so the first turn of every session is also captured.
        self._lat_audio_marked_for_turn: int | None = None
...
def build_app(args: argparse.Namespace) -> FastAPI:
    ...
    async def video_stream(ws: WebSocket):
        ...
        # AVATAR_FIRST_FRAME tracking: pulled from the session dict so this
        # coroutine never holds a direct reference to reply_engine.
        _lat_logger = session.get("lat_logger")
        _lat_get_turn_id = session.get("lat_get_turn_id")
        _lat_avatar_last_turn_id: int | None = None
        print(
        ...
        try:
            while True:
                ...
                # AVATAR_FIRST_FRAME: mark the instant the first video frame of
                # each turn is actually sent. Fires once per turn using turn_id
                # comparison so it resets automatically on each new question.
                if _lat_logger is not None and _lat_get_turn_id is not None:
                    _cur_vid_turn = _lat_get_turn_id()
                    if (
                        _cur_vid_turn is not None
                        and _cur_vid_turn != _lat_avatar_last_turn_id
                    ):
                        _lat_avatar_last_turn_id = _cur_vid_turn
                        with contextlib.suppress(Exception):
                            _lat_logger.mark(
                                _cur_vid_turn, "avatar_first_frame",
                                f"frame_q={frame_q.qsize()} frame_idx={idx}",
                            )
                            _lat_logger.count(
                                _cur_vid_turn,
                                frame_queue_depth_at_first_frame=frame_q.qsize(),
                            )
    ...
    async def conversation(ws: WebSocket):
        ...
        if reply_engine is not None:
            ...
            split_sessions[session_id] = {
                ...
                # Latency logger reference used by /ws/video to mark AVATAR_FIRST_FRAME.
                # Lambda read is safe across threads: _latency_turn_id is a plain int
                # attribute on the GPU thread, protected by Python's GIL for reads.
                "lat_logger": reply_engine.conv_logger.latency,
                "lat_get_turn_id": lambda: reply_engine._latency_turn_id,
            }
        ...
        if reply_engine is not None:
            def _gpu_producer_thread() -> None:
                ...
                while not stopped:
                    ...
                    for ev in reply_engine.process_ready_steps_limited(max_moshi_steps_per_loop):
                        ...
                        # AUDIO_FIRST_CHUNK_READY + AUDIO_QUEUE_ENTER: fired once per
                        # turn (when _latency_turn_id advances) on the very first audio
                        # packet. All subsequent packets for the same turn skip the check
                        # cheaply. Runs on the GPU thread so no locking is needed.
                        _lat_turn_now = getattr(reply_engine, "_latency_turn_id", None)
                        _lat_audio_prev = getattr(reply_engine, "_lat_audio_marked_for_turn", None)
                        _lat_fire_audio_mark = (
                            _lat_turn_now is not None
                            and _lat_turn_now != _lat_audio_prev
                        )
                        if _lat_fire_audio_mark:
                            reply_engine._lat_audio_marked_for_turn = _lat_turn_now
                        ...
                        for frame_offset, audio_slice in enumerate(frame_audio[:n_frames]):
                            ...
                            # depth reading reflects the actual backlog at that moment.
                            if _lat_fire_audio_mark and frame_offset == 0:
                                _lat_fire_audio_mark = False  # guard against re-entry
                                with contextlib.suppress(Exception):
                                    _aq_depth = audio_q.qsize() if audio_q is not None else -1
                                    _fq_depth = frame_q.qsize() if frame_q is not None else -1
                                    _backlog = round(reply_engine.input_backlog_sec(), 3)
                                    _lat = reply_engine.conv_logger.latency
                                    _lat.mark(
                                        _lat_turn_now, "audio_first_chunk_ready",
                                        f"audio_q={_aq_depth} frame_q={_fq_depth}",
                                    )
                                    _lat.mark(
                                        _lat_turn_now, "audio_queue_enter",
                                        f"frame={emitted + frame_offset}",
                                    )
                                    _lat.count(
                                        _lat_turn_now,
                                        audio_queue_depth=_aq_depth,
                                        frame_queue_depth=_fq_depth,
                                        input_backlog=_backlog,
                                        pending_audio_duration=round(
                                            _aq_depth / float(args.fps), 3
                                        ),
                                    )
            ...
            async def _audio_sender() -> None:
                ...
                # AUDIO_PLAYBACK_START tracking: fires once per turn on the
                # first successful ws.send_bytes(). Tracked by turn_id so it
                # resets automatically when the engine opens a new turn.
                _lat_audio_play_last_turn_id: int | None = None
                ...
                while True:
                    ...
                    if opus_payload:
                        ...
                        if _is_first_chunk:
                            ...
                            with contextlib.suppress(Exception):
                                _aq = audio_q.qsize()
                                _fq = frame_q.qsize() if frame_q is not None else -1
                                ...
                                _lat.count(
                                    _cur_turn_id,
                                    audio_queue_depth_at_playback=_aq,
                                    frame_queue_depth_at_playback=_fq,
                                    input_backlog_at_playback=_bl,
                                    pending_audio_duration=round(
